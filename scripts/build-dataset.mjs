...
const SPECIAL_DCI = new Map([
  ['ACÉTYLSALICYLIQUE (ACIDE)', 'ACIDE ACÉTYLSALICYLIQUE'],
  ['DL-LYSINE (ACÉTYLSALICYLATE DE)', 'ACÉTYLSALICYLATE DE LYSINE'],
  ['ACÉTYLSALICYLATE DE DL-LYSINE', 'ACÉTYLSALICYLATE DE LYSINE'],
  ['CLAVULANATE DE POTASSIUM', 'ACIDE CLAVULANIQUE'],
  ['CLAVULANATE POTASSIQUE', 'ACIDE CLAVULANIQUE'],
  ['COMPLEXE SODIQUE SACUBITRIL VALSARTAN HÉMIPENTAHYDRATÉ', 'SACUBITRIL + VALSARTAN'],
  ["OPIUM (EXTRAIT SEC D')", 'OPIUM'],
  ["POUDRE D'OPIUM", 'OPIUM'],
  ['SODIUM (VALPROATE DE)', 'ACIDE VALPROÏQUE'],
  ['VALPROATE DE SODIUM', 'ACIDE VALPROÏQUE'],
  ['DIVALPROATE DE SODIUM', 'ACIDE VALPROÏQUE'],
  ['PHLOROGLUCINOL HYDRATÉ', 'PHLOROGLUCINOL'],
  ["PHLOROGLUCINOL (TRIMÉTHYL ÉTHER DU)", 'TRIMÉTHYLPHLOROGLUCINOL'],
  ["FERREUX (SULFATE) DESSÉCHÉ", 'SULFATE FERREUX'],
  ['SULFATE FERREUX DESSÉCHÉ', 'SULFATE FERREUX'],
  ['POTASSIUM (CHLORURE DE)', 'CHLORURE DE POTASSIUM'],
  ['SODIUM (ALGINATE DE)', 'ALGINATE DE SODIUM'],
  ['SODIUM (BICARBONATE DE)', 'BICARBONATE DE SODIUM'],
  ['ÉSOMÉPRAZOLE MAGNÉSIUM', 'ÉSOMÉPRAZOLE'],
  ['PANTOPRAZOLE SODIQUE SESQUIHYDRATÉ', 'PANTOPRAZOLE'],
  ['INSULINE LISPRO RECOMBINANTE', 'INSULINE LISPRO'],
  ['ROSUVASTATINE ZINCIQUE', 'ROSUVASTATINE'],
  ['CLINDAMYCINE (CHLORHYDRATE DE) HYDRATÉ', 'CLINDAMYCINE'],
  ['CLINDAMYCINE HYDRATÉ', 'CLINDAMYCINE'],
  ['BÉTAMETHASONE (PHOSPHATE DE) ET DE DISODIUM', 'BÉTAMÉTHASONE'],
  ['CYAMÉPROMAZINE', 'CYAMÉMAZINE'],
  ['MALÉATE ACIDE DE LÉVOMÉPROMAZINE', 'LÉVOMÉPROMAZINE'],
  ['PRAMIPEXOLE (DICHLORHYDRATE DE)', 'PRAMIPEXOLE'],
  ['PIPAMPÉRONE (DICHLORHYDRATE DE)', 'PIPAMPÉRONE'],
  ['DICHLORHYDRATE DE ZUCLOPENTHIXOL', 'ZUCLOPENTHIXOL'],
  ['ZUCLOPENTHIXOL (DICHLORHYDRATE DE)', 'ZUCLOPENTHIXOL'],
  ["AMILORIDE (CHLORHYDRATE D') HYDRATÉ", 'AMILORIDE'],
  ['BROMHYDRATE DE DEXTROMÉTHORPHANE', 'DEXTROMÉTHORPHANE'],
  ['BROMHYDRATE DE FÉNOTÉROL', 'FÉNOTÉROL'],
  ['MÉTHYLDOPA SESQUIHYDRATÉ', 'MÉTHYLDOPA'],
  ['OXYCODONE (CHLORHYDRATE D\')', 'OXYCODONE'],
  ['FUSIDIQUE (ACIDE)', 'ACIDE FUSIDIQUE'],
  ['SODIUM (FUSIDATE DE)', 'ACIDE FUSIDIQUE'],
  ['SODIUM (FÉRÉDÉTATE DE)', 'FÉRÉDÉTATE DE SODIUM'],
  ['BENZALKONIUM (CHLORURE DE)', 'CHLORURE DE BENZALKONIUM'],
  ['CHLORHEXIDINE (DIGLUCONATE DE)', 'CHLORHEXIDINE'],
]);
function cleanDci(dci) {
  ...
  while (previous !== s) {
    previous = s;
    s = s.replace(/\s*\((CHLORHYDRATE|AC[ÉE]TATE|FUMARATE|SULFATE|HYDROG[ÉE]NOSULFATE|MAL[ÉE]ATE|TARTRATE|PHOSPHATE|GLUCONATE|B[ÉE]SILATE|HYDROBROMURE|M[ÉE]SILATE|BROMURE|IODURE|CITRATE|NITRATE|OXALATE|LACTATE|BENZOATE|SUCCINATE|ST[ÉE]ARATE|D[ÉE]CANOATE|PALMITATE|PIVALATE|VAL[ÉE]RATE|XINAFOATE|CIPIONATE|DIPROPIONATE|PROPIONATE|FUROATE)\s+(DE|D')\)\s*$/i, '');
    s = s.replace(SALT_ADJ, '');
    s = s.replace(SALT_PREFIX, '');
  }
  const cleaned = s.trim();
  return SPECIAL_DCI.get(cleaned.toUpperCase()) ?? cleaned;
}

function cleanDciSet(dcis) {
  const byComparable = new Map();
  for (const raw of dcis) {
    const cleanedParts = cleanDci(raw).split(/\s+\+\s+/);
    for (const dci of cleanedParts) {
      const key = comparable(dci);
      if (!key) continue;
      // Keep the shortest readable label when several salt/hydrate variants collapse.
      const previous = byComparable.get(key);
      if (!previous || dci.length < previous.length) byComparable.set(key, dci);
    }
  }
  return new Set(byComparable.values());
}
...
function isPoorQuizCandidate(med) {
  ...
  if (/^A11|^A12AA|^A12AX/.test(code)) return true; // Keep only explicitly selected vitamins/minerals.
  if (/^A06A/.test(code) && !/^(TRANSIPEG|MOVICOL|DUPHALAC)\b/i.test(name)) return true;
  if (/^A02A/.test(code)) return true; // Simple antacids are less discriminating than IPP/Gaviscon.
  if (/^V03AE/.test(code)) return true;
